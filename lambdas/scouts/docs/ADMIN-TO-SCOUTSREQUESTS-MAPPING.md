...
| Persist tagline | `realm: 'scouts'`, `subject: { hexId, tagline }`, `action: 'persist'` | `realm: 'persist'`, `action: 'persist'`, `subject: { hexId, tagline }` | Forwarded to `scoutsProcessing` as `persist/persist` | The current admin page sends a structured subject object, not `subject: 'tagline'`. |
| Persist image theme | `realm: 'scouts'`, `subject: { hexId, imageTheme }`, `action: 'persist'` | `realm: 'persist'`, `action: 'persist'`, `subject: { hexId, imageTheme }` | Forwarded to `scoutsProcessing` as `persist/persist` | `imageTheme` is the active field name in the current flow. |
| Persist image URL | `realm: 'scouts'`, `subject: { hexId, imageUrl }`, `action: 'persist'` | `realm: 'persist'`, `action: 'persist'`, `subject: { hexId, imageUrl }` | Forwarded to `scoutsProcessing` as `persist/persist` | URL validation happens in `scouts` before queueing. |
| Generate tagline | `realm: 'scouts'`, `subject: { hexId }`, `action: 'generateTagline'` | `realm: 'tagline'`, `action: 'request'`, `subject: '<hex>'` | Forwarded unchanged to `scoutsProcessing` | The admin button passes the field-specific action token. |
| Generate image theme | `realm: 'scouts'`, `subject: { hexId }`, `action: 'generateImageTheme'` | `realm: 'imageTheme'`, `action: 'request'`, `subject: '<hex>'` | Forwarded unchanged to `scoutsProcessing` | Current flow uses `imageTheme`, not `imagePrompt` or `pixabay`. |
| Generate image | `realm: 'scouts'`, `subject: { hexId }`, `action: 'generateImage'` | `realm: 'image'`, `action: 'request'`, `subject: '<hex>'` | Forwarded unchanged to `scoutsProcessing` | Current flow uses `image`, not `pixabay`. |
