...
QUEUE_ARN="${QUEUE_ARN:-arn:aws:sqs:${REGION}:${EXPECTED_AWS_ACCOUNT}:scoutsProcessing}"
QUEUE_URL="${QUEUE_URL:-https://sqs.${REGION}.amazonaws.com/${EXPECTED_AWS_ACCOUNT}/scoutsProcessing}"
SCOUTS_DECISION_QUEUE_ARN="${SCOUTS_DECISION_QUEUE_ARN:-arn:aws:sqs:${REGION}:${EXPECTED_AWS_ACCOUNT}:scoutsDecision}"
SCOUTS_DECISION_QUEUE_URL="${SCOUTS_DECISION_QUEUE_URL:-https://sqs.${REGION}.amazonaws.com/${EXPECTED_AWS_ACCOUNT}/scoutsDecision}"
DLQ_ARN="${DLQ_ARN:-arn:aws:sqs:${REGION}:${EXPECTED_AWS_ACCOUNT}:scoutsProcessingDLQ}"
DLQ_URL="${DLQ_URL:-https://sqs.${REGION}.amazonaws.com/${EXPECTED_AWS_ACCOUNT}/scoutsProcessingDLQ}"
...
CLOUDFLARE_AI_MODEL="${CLOUDFLARE_AI_MODEL:-}"
...
TARGET_BUCKET="${TARGET_BUCKET:-${WEBSITE_BUCKET:-}}"
...
S3_WEBSITE_BASE_URL="${S3_WEBSITE_BASE_URL:-https://${TARGET_BUCKET}.s3.${REGION}.amazonaws.com}"

: "${CODE_BUCKET:?CODE_BUCKET must be set}"
: "${EXPECTED_AWS_ACCOUNT:?EXPECTED_AWS_ACCOUNT must be set}"
: "${TARGET_BUCKET:?TARGET_BUCKET or WEBSITE_BUCKET must be set}"
...
if [ -z "${IMAGE_GENERATION_PROVIDER}" ]; then
  CURRENT_IMAGE_GENERATION_PROVIDER="$(aws lambda get-function-configuration --function-name "${FUNCTION_NAME}" --region "${REGION}" --query 'Environment.Variables.IMAGE_GENERATION_PROVIDER' --output text 2>/dev/null || true)"
  case "${CURRENT_IMAGE_GENERATION_PROVIDER}" in
    disabled|cloudflare|gemini) IMAGE_GENERATION_PROVIDER="${CURRENT_IMAGE_GENERATION_PROVIDER}" ;;
    *) IMAGE_GENERATION_PROVIDER="disabled" ;;
  esac
fi
if [ -z "${CLOUDFLARE_ACCOUNT_ID}" ]; then
  CURRENT_CLOUDFLARE_ACCOUNT_ID="$(aws lambda get-function-configuration --function-name "${FUNCTION_NAME}" --region "${REGION}" --query 'Environment.Variables.CLOUDFLARE_ACCOUNT_ID' --output text 2>/dev/null || true)"
  if [ -n "${CURRENT_CLOUDFLARE_ACCOUNT_ID}" ] && [ "${CURRENT_CLOUDFLARE_ACCOUNT_ID}" != "None" ] && [ "${CURRENT_CLOUDFLARE_ACCOUNT_ID}" != "null" ]; then CLOUDFLARE_ACCOUNT_ID="${CURRENT_CLOUDFLARE_ACCOUNT_ID}"; fi
fi
if [ -z "${CLOUDFLARE_AI_MODEL}" ]; then
  CURRENT_CLOUDFLARE_AI_MODEL="$(aws lambda get-function-configuration --function-name "${FUNCTION_NAME}" --region "${REGION}" --query 'Environment.Variables.CLOUDFLARE_AI_MODEL' --output text 2>/dev/null || true)"
  if [ -n "${CURRENT_CLOUDFLARE_AI_MODEL}" ] && [ "${CURRENT_CLOUDFLARE_AI_MODEL}" != "None" ] && [ "${CURRENT_CLOUDFLARE_AI_MODEL}" != "null" ]; then CLOUDFLARE_AI_MODEL="${CURRENT_CLOUDFLARE_AI_MODEL}"; else CLOUDFLARE_AI_MODEL="@cf/black-forest-labs/flux-1-schnell"; fi
fi
...
CFN_DEPLOY_ARGS+=(
  --parameter-overrides
    ...
    SlackWebhookUrl="${SLACK_WEBHOOK_URL}" ScoutsNotificationChannel="${SCOUTS_NOTIFICATION_CHANNEL}"
