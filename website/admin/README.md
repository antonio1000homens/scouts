...
Visibility is occurrence-owned state. Shared metadata publication must preserve an occurrence's current visibility unless the request explicitly targets that `occurrenceId` with a visibility change. Review/approval may reuse shared HEX metadata, but any visibility presented with that review must refer to the selected occurrence. UI pending state is also action-scoped, so one accepted or slow operation must not disable unrelated event actions.
